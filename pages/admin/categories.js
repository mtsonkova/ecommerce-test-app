...
export default function AdminCategories() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        showAlert(editingCategory ? 'Category updated!' : 'Category created!', 'success');
        setShowModal(false);
        resetForm();
        fetchCategories();
      } else {
  ...
  const handleDelete = (categoryId) => {
    showConfirm('Are you sure you want to delete this category? This will fail if there are products in this category.', async () => {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={() => { resetForm(); setShowModal(true); }} className="bg-indigo-600 text-white px-6 py-3 rounded-md hover:bg-indigo-700 font-semibold">
            + Add Category
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white rounded-lg shadow-md">
            ...
            <button onClick={() => setShowModal(true)} className="bg-indigo-600 text-white px-6 py-3 rounded-md hover:bg-indigo-700 font-semibold">Create Category</button>
          </div>
        ) : (
            ...
            {categories.map(category => (
              <div key={category.id} className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="bg-indigo-100 text-indigo-800 text-sm font-medium px-3 py-1 rounded-full">{category._count.products} products</span>
                </div>
                <h3 className="text-xl font-semibold text-gray-900 mb-2">{category.name}</h3>
                {category.description && <p className="text-gray-600 text-sm mb-4">{category.description}</p>}
                <div className="flex space-x-2 mt-4">
                  <button onClick={() => handleEdit(category)} className="flex-1 px-4 py-2 bg-indigo-100 text-indigo-700 rounded-md hover:bg-indigo-200 text-sm font-medium">Edit</button>
                  <button
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" required placeholder="e.g., Electronics, Clothing, Books" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Description (optional)</label>
                <textarea rows="3" placeholder="Brief description of this category" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} />
              </div>
              <div className="flex space-x-3 pt-4">
                ...
                <button type="submit" className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">{editingCategory ? 'Update' : 'Create'}</button>
              </div>
            </form>
          </div>
        </div>
