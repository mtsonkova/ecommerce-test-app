  ...
  const handleProcessRefund = async (refundId, action) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        alert(data.error || `Failed to ${actionText} refund`);
      }
    } catch (err) {
      alert(`Failed to ${actionText} refund`);
    }
                  ...
                  <div className="space-y-3">
                    {refund.order.orderItems.map(item => (
                      <div key={item.id} className="flex items-center justify-between pb-3 border-b last:border-b-0">
                        <div className="flex items-center space-x-4">
                          <div className="w-12 h-12 bg-gradient-to-br from-indigo-100 to-purple-100 rounded flex items-center justify-center text-2xl">
                            📦
                          </div>
                          <div>
                  ...
                  <div className={`px-6 py-3 border-t ${
                    ...
                    <p className="text-sm">
                      {refund.status === 'approved' 
                        ? '✅ Refund approved and processed. Customer will receive funds within 3-5 business days.'
                        : '❌ Refund request has been rejected.'}
                    </p>
                    <p className="text-xs text-gray-600 mt-1">
                      Processed: {new Date(refund.updatedAt).toLocaleString()}
                    </p>
                  </div>
                )}
              </div>
