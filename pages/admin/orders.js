...
export default function AdminOrders() {
  ...
  useEffect(() => { fetchUser(); fetchOrders(); }, [statusFilter, paymentFilter]);
  ...
  const handleStatusChange = (orderId, newStatus) => {
    showConfirm(`Are you sure you want to change this order status to "${newStatus}"?`, async () => {
  ...
  const getStatusColor = (status) => {
    const colors = { pending: 'bg-yellow-100 text-yellow-800', processing: 'bg-blue-100 text-blue-800', shipped: 'bg-purple-100 text-purple-800', delivered: 'bg-green-100 text-green-800', cancelled: 'bg-red-100 text-red-800', returned: 'bg-gray-100 text-gray-800' };
    return colors[status] || 'bg-gray-100 text-gray-800';
  };

  const getPaymentStatusColor = (status) => {
    const colors = { pending: 'bg-yellow-100 text-yellow-800', paid: 'bg-green-100 text-green-800', refunded: 'bg-blue-100 text-blue-800' };
    return colors[status] || 'bg-gray-100 text-gray-800';
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <ConfirmModal isOpen={confirmModal.isOpen} message={confirmModal.message} confirmText="Update Status" confirmVariant="primary" onConfirm={confirmModal.onConfirm} onCancel={() => setConfirmModal(prev => ({ ...prev, isOpen: false }))} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link href="/admin/refunds" className="text-indigo-600 hover:text-indigo-800">Refunds</Link>
              {user && (
                <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" value={paymentFilter} onChange={(e) => setPaymentFilter(e.target.value)}>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white rounded-lg shadow-md">No orders found</div>
        ) : (
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(order.status)}`}>{order.status.charAt(0).toUpperCase() + order.status.slice(1)}</span>
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${getPaymentStatusColor(order.paymentStatus)}`}>{order.paymentStatus.charAt(0).toUpperCase() + order.paymentStatus.slice(1)}</span>
                      </div>
                ...
                <div className="px-6 py-4 bg-gray-50 border-t">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">{order.paymentMethod} ending in {order.cardLast4}</p>
                  ...
                  <div className="px-6 py-4 bg-yellow-50 border-t">
                    ...
                    {order.refunds.map(refund => (
                      <div key={refund.id} className="text-sm space-y-1">
                        <p>Status: <span className={`font-semibold ${refund.status === 'approved' ? 'text-green-600' : refund.status === 'rejected' ? 'text-red-600' : 'text-yellow-600'}`}>{refund.status.charAt(0).toUpperCase() + refund.status.slice(1)}</span></p>
